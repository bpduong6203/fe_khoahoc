...
const FeaturedCourses: React.FC<FeaturedCoursesProps> = ({ coursesByCategory }) => {
  return (
    <div className="px-6 py-12 max-w-6xl mx-auto">
      ...
      <p className="dark:text-neutral-200 mt-2 text-lg">Các khóa học được đánh giá cao và phổ biến nhất</p>
         ...
         coursesByCategory["TECHNOLOGY & SOFTWARE"].slice(0, 4).map((course, index) => (
          <div key={index} className="border rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden flex">
            ...
            <div className="w-2/3 p-4 bg-neutral-200 dark:bg-neutral-400">
              <h3 className="font-semibold text-gray-800 dark:text-neutral-50 line-clamp-2">{course.title}</h3>
              <p className="text-sm text-gray-500 dark:text-neutral-100 mt-2">{course.author}</p>
              <div className="flex items-center mt-2">
                <div className="flex items-center">
                  <span className="text-yellow-400 mr-1">★</span>
                  <span className="text-sm font-medium">{course.rating}</span>
                </div>
                <span className="text-xs text-gray-500 dark:text-neutral-200 ml-1">({course.participants})</span>
              </div>
              <p className="text-sm font-bold text-gray-800 dark:text-neutral-100 mt-2">{course.price}</p>
            </div>
          </div>
        ))}
      </div>
